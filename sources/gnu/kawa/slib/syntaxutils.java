...
public class syntaxutils extends ModuleBody {
        ...
        if (exp instanceof LetExp) {
            ...
            try {
                ...
                for (Declaration decl = exp3.firstDecl(); decl != null; decl = decl.nextDecl()) {
                    closureEnv.pack = C0605lists.cons(decl.getSymbol(), closureEnv.pack);
                }
                objArr2[0] = C0605lists.reverse$Ex(closureEnv.pack);
    ...
    static Object unrewrite$St(Expression[] exps) {
        ...
        for (Object obj = Lit7; Scheme.numEqu.apply2(obj, len) == Boolean.FALSE; obj = AddOp.$Pl.apply2(obj, Lit8)) {
            closureEnv.pack = C0605lists.cons(unrewrite(exps[((Number) obj).intValue()]), closureEnv.pack);
        }
        return C0605lists.reverse$Ex(closureEnv.pack);
    }
    ...
    static Object unrewriteLet(LetExp exp) {
        ...
        Object obj = Lit7;
        while (decl != null) {
            closureEnv.pack = C0605lists.cons(LList.list2(decl.getSymbol(), unrewrite(exp.inits[((Number) obj).intValue()])), closureEnv.pack);
            decl = decl.nextDecl();
