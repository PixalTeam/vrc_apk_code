...
public abstract class Sprite extends VisibleComponent implements AlarmHandler, OnDestroyListener, Deleteable {
    ...
    @SimpleProperty(category = PropertyCategory.APPEARANCE)
    /* renamed from: Z */
    public void mo7732Z(double layer) {
        this.zLayer = layer;
    ...
    public void PointTowards(Sprite target) {
        Heading(-Math.toDegrees(Math.atan2((target.mo7729Y() - mo7729Y()) + ((double) ((target.Height() - Height()) / 2)), (target.mo7727X() - mo7727X()) + ((double) ((target.Width() - Width()) / 2)))));
    }
    ...
    public void PointInDirection(double x, double y) {
        Heading(-Math.toDegrees(Math.atan2((y - mo7729Y()) - ((double) (Height() / 2)), (x - mo7727X()) - ((double) (Width() / 2)))));
    }
    ...
    public BoundingBox getBoundingBox(int border) {
        return new BoundingBox(mo7727X() - ((double) border), mo7729Y() - ((double) border), ((mo7727X() + ((double) Width())) - DEFAULT_Z) + ((double) border), ((mo7729Y() + ((double) Height())) - DEFAULT_Z) + ((double) border));
    }
